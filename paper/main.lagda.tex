...
arguments.\footnote{
  ...
  to be supplied by the user. The implementation of |auto|
  identifies it with Agda's |Name| extended with constructors for
  variable names and function types.
}
...
\wouter{Give an example of such a rule. For instance: | add(0,x,x)| and |add(n,m,t)
  :- add (suc(n), m, suc(t))|.}
\pepijn{Will do. Don't you think that this example is a bit too long?
  Maybe just the base or the step rule?}
\todo{mention usage of the above example's |TermName| datatype}
